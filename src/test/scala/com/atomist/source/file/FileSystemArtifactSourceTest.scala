...
import com.atomist.util.{AtomistIgnoreFileFilter, GitignorePathFilterOld, GitignorePathFilter}
...
//    fWriter.write(zipSource, fid, SimpleSourceUpdateInfo(getClass.getName))
//
//    val filter = GitignorePathFilter(tmpDir.toString)
//    val as = FileSystemArtifactSource(fid, Seq(filter))
//    as.findDirectory(".atomist/node_modules") shouldBe defined
//  }
//
...
//    fWriter.write(zipSource, fid, SimpleSourceUpdateInfo(getClass.getName))
//
//    val filter = AtomistIgnorePathFilter(tmpDir.toString)
//    val as = FileSystemArtifactSource(fid, Seq(filter))
...
//  }
  ...
  it should "handle filtering .git and target from artifact-source" in  {
    ...
    val gitignoreFilter = GitignorePathFilter(rootPath)
    val filter = AtomistIgnoreFileFilter(rootPath)
    val as = FileSystemArtifactSource(fid, Seq(gitignoreFilter))
    as.allFiles.size should be > 10
